...
# 初始化模型
model_sample_model = YOLO("./model/yolov8n.pt")
...
def get_bytes_from_image(image: Image.Image) -> BytesIO:
    """
    将PIL图像转换为字节
...
def add_bboxs_on_img(image: Image.Image, predict: pd.DataFrame) -> Image.Image:
    """
    在图像上添加边界框
...
def detect_sample_model(input_image: Image.Image) -> pd.DataFrame:
    """
    使用sample_model进行预测。
    基于YoloV8
...
def class_sample_model(input_image: Image.Image) -> pd.DataFrame:
    """
    使用sample_model进行分类。
    基于YoloV8
